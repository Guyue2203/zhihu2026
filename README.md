...
返回 `accept`、`clarify` 或 `reject`。系统会先寻找原问题里的隐含时间线：同一主题可以自然形成阶段变化时自动改写为带时间语义的检索问题，并在前端展示；只有算式、即时任务、固定事实等明显没有相关时间线的问题才拒绝。存在多个差异很大的时间线方向时返回最多三个选项。主接口会重复执行同一校验，不能通过跳过前端绕开。
...
|---|---|
| `query` | 规范化后的用户问题 |
| `retrievalQuery` | 实际用于规划与检索的问题；发现隐含时间线时为自动改写后的问题，否则与 `query` 相同 |
| `title` | 时间线标题 |
