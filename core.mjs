...
export async function buildJourney(rawQuery, mode = process.env.BACKEND_MODE || 'live') {
  ...
  const summary = await deepseekJson(summaryPrompt, { query, plan: { ...plan, stages: crawledStages }, posts: uniquePosts.map(({ id, question, answerer, excerpt, url, votes, comments, heat, stageId }) => ({ id, question, answerer, excerpt, url, votes, comments, heat, stageId })) }, 5000);
  ...
  return { query, title: String(summary.title || plan.title || query), thesis: String(summary.thesis || plan.thesis || '待验证'), stages, posts, limitations: [...(Array.isArray(summary.limitations) ? summary.limitations.filter(Boolean).slice(0, 6) : []), '每个阶段的帖子来自有限检索样本；热度分数只用于排序，不代表真实性。'], coverage: 'sampled', evidenceCount: uniquePosts.length, selectedCount: posts.length };
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href && process.argv[2] === '--self-test') selfTest();
